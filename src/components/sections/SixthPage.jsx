...
const SixthPage = () => {
  ...
  const baseScale = isMobile ? 1.0 : 0.8;
  const lastScale = isMobile ? 1.2 : 1.0;
  ...
  }, [isMobile]); // refresh when scale changes with resize
  ...
  useEffect(() => {
    ...
    const proxy = { p: 0 };
    const smoothTo = gsap.quickTo(proxy, "p", { duration: 0.25, ease: "none" });
    ...
    const st = ScrollTrigger.create({
      ...
      fastScrollEnd: true,
      onUpdate: (self) => {
        smoothTo(self.progress);
        const idx = Math.min(
          totalSteps - 1,
          Math.floor(proxy.p * (totalSteps - 0.0001))
        );
        if (idx !== currentIndexRef.current && !isAnimatingRef.current) {
          swapTo(idx);
        }
  ...
  const swapTo = (nextIndex) => {
    ...
    const prev = phases[currentIndexRef.current];

    const tl = gsap.timeline({
      defaults: { duration: 0.32, ease: "none" },
      onComplete: () => {
        if (imageBottomRef.current && imageTopRef.current) {
          imageBottomRef.current.src = next.img;
          gsap.set(imageBottomRef.current, { opacity: 1, scale: next.scale });
          gsap.set(imageTopRef.current, { opacity: 0, scale: 1 });
        }
        currentIndexRef.current = nextIndex;
        isAnimatingRef.current = false;
    ...
    gsap.set(textWrapRef.current, { willChange: "transform, opacity" });

    tl.to(textWrapRef.current, { opacity: 0, y: -6 }, 0);

    tl.add(() => {
      setPhaseIndex(nextIndex);
      if (imageTopRef.current) {
        imageTopRef.current.src = next.img;
      }
    });
    ...
    tl.set(imageTopRef.current, { opacity: 0, scale: next.scale * 0.95 }, 0);
    tl.to(imageTopRef.current, { opacity: 1, scale: next.scale }, 0.05);
    tl.to(
      imageBottomRef.current,
      { opacity: 0, scale: (prev?.scale ?? baseScale) * 1.02 },
      0.05
    );
  };
          ...
          <div className="relative z-10 w-full md:w-1/2 flex justify-center md:justify-end items-start mt-10 md:mt-[1rem]">
            <div className="relative w-full max-w-[640px] md:max-w-[560px] lg:max-w-[640px] aspect-[3/4] min-h-[300px]">
              ...
              <img
                ref={imageBottomRef}
                alt="evolution"
                className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-full w-full object-contain select-none block transform-gpu will-change-transform"
                draggable={false}
                decoding="async"
                loading="eager"
                style={{ opacity: 1 }}
              ...
              <img
                ref={imageTopRef}
                alt="evolution transition"
                className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 h-full w-full object-contain opacity-0 select-none block transform-gpu will-change-transform"
                draggable={false}
                decoding="async"
                loading="eager"
                style={{ opacity: 0 }}
